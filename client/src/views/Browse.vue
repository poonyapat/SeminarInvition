    ...
    <v-container fluid>
        <v-layout row wrap justify-center>
            <v-flex xs12 sm10 md8 lg6 text-xs-center>
                <v-text-field clearable outline label="Search" v-model="searcher" prepend-inner-icon="search">
                </v-text-field>
                <v-pagination v-if="seminars.length" v-model="page" :length="maxPage" :total-visible="7" round></v-pagination>
    ...
    export default {
        ...
        watch: {
            ...
            '$route.query.search': {
                ...
                async handler(value) {
                    this.searcher = value
                    let temp = (await SeminarService.findAll({
                        search: value,
                        page: this.page
                    })).data
